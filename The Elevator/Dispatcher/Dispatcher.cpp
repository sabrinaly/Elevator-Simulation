...
int main()
{
	...
	r1.Wait();
	CTypedPipe<pipeline_data> dispatcherPipe("DispatcherPipeline", 1024); // pipeline from IO with keyboard commands
	int Message = NULL;
		...
		if (mystruct.x == 'u')
		{
			...
			if (current_floor >= 0 && current_floor <= 9)
			{
				...
				if (E1_status.direction && E1_status.floor <= current_floor && E1_status.target_floor >= current_floor)
				{
					...
					p2.Post(Message);
				else if (E1_status.direction && E1_status.floor <= current_floor)
				{ // target_floor is < current_floor
